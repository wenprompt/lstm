...
class DataBuildingValidator:
    ...
    def validate_m1_contract_mapping(self) -> Dict[str, bool]:
        ...
        for year, month, expected_year, expected_month in test_cases:
            ...
            is_correct = (
                contract_year == expected_year and contract_month == expected_month
            )
            ...
            logger.info(
                f"  {status} {test_name}: got {contract_year}-{contract_month:02d}"
            )

        all_correct = all(results.values())
        logger.info(
            f"{'✅' if all_correct else '❌'} M+1 mapping logic validation: {'PASSED' if all_correct else 'FAILED'}"
        )
        return results
    ...
    def validate_continuous_futures_output(self) -> Dict[str, any]:
        ...
        validation_results = {
            ...
            "no_missing_prices": df[["price_65_m1", "price_62_m1"]].isnull().sum().sum()
            == 0,
        }
        ...
        validation_results.update(
            {
                ...
                "price_continuity_ok": extreme_changes_65 == 0
                and extreme_changes_62 == 0,
            }
        )
        ...
        logger.info(
            f"  Date range: {validation_results['date_range'][0]} to {validation_results['date_range'][1]}"
        )
        ...
        logger.info(
            f"  ✅ Chronological order: {validation_results['chronological_order']}"
        )
        logger.info(
            f"  ✅ No missing prices: {validation_results['no_missing_prices']}"
        )
        logger.info(
            f"  ✅ Price continuity: {validation_results['price_continuity_ok']} (extreme changes: 65%={extreme_changes_65}, 62%={extreme_changes_62})"
        )
    ...
    def validate_date_parsing_consistency(self) -> Dict[str, any]:
            ...
            if "Date" in group_df.columns:
                ...
                try:
                    parsed = pd.to_datetime(
                        sample_date, format="%d/%m/%Y", dayfirst=True
                    )
        ...
        for file_name, info in date_formats.items():
            ...
            logger.info(
                f"  {status} {file_name}: {info['format']} - sample: '{info['sample']}'"
            )
            if not info.get("parse_success", False):
                all_successful = False

        logger.info(
            f"{'✅' if all_successful else '❌'} Date parsing consistency: {'PASSED' if all_successful else 'FAILED'}"
        )
    ...
    def validate_feature_alignment(self) -> Dict[str, any]:
        ...
        if not consolidated_file.exists():
            logger.error(
                f"❌ Consolidated features file not found: {consolidated_file}"
            )
            return {"file_exists": False}
        ...
        expected_features = [
            "price_65_m1",
            "price_62_m1",  # From continuous futures
            "Ukraine Concentrate fines",
            "lump premium",
            "IOCJ Import margin",
            "rebar steel margin ",
            "indian pellet premium",
            "(IOCJ+SSF)/2-PBF",  # From group.csv
            "62 Index",
            "65 Index",  # From index file
            "IOCJ Inventory",
            "IOCJ Weekly shipment",  # From weekly files
        ]
        ...
        alignment_results = {
            ...
            "missing_features": [
                f for f in expected_features if f not in actual_features
            ],
            "extra_features": [
                f for f in actual_features if f not in expected_features
            ],
        ...
        if "price_65_m1" in df.columns and "Y" in df.columns:
            ...
            expected_y = (price_65.shift(-1) / price_65).apply(
            ...
            if valid_mask.sum() > 0:
                ...
                max_diff = abs(
                    actual_y[valid_mask] - expected_y_trimmed[valid_mask]
                ).max()
                alignment_results["y_calculation_correct"] = (
                    correlation > 0.999 and max_diff < 0.001
                )
        ...
        if alignment_results["missing_features"]:
            logger.error(
                f"  ❌ Missing features: {alignment_results['missing_features']}"
            )
    ...
    def validate_price_continuity(self) -> Dict[str, any]:
        ...
        logger.info(
            f"  ✅ Price continuity: {'GOOD' if continuity_results['price_continuity_good'] else 'ISSUES DETECTED'}"
        )
        logger.info(
            f"     Max daily return: 65%={continuity_results['max_return_65']:.4f}, 62%={continuity_results['max_return_62']:.4f}"
        )
    ...
    def validate_target_variable_calculation(self) -> Dict[str, any]:
        ...
        logger.info(
            f"     Valid Y values: {y_stats['valid_y_count']}/{y_stats['total_y_count']}"
        )
        ...
        logger.info(
            f"  ✅ Calculation accurate: {target_results['calculation_accurate']}"
        )
    ...
    def validate_data_file_integrity(self) -> Dict[str, any]:
        ...
        for filename, description in required_files.items():
            ...
            if file_path.exists():
                try:
                    if filename.endswith(".pkl"):
                        ...
                        file_integrity[filename]["has_date_column"] = (
            ...
            if file_integrity[filename]["exists"]:
                ...
                if loadable and "shape" in file_integrity[filename]:
                    ...
                    logger.info(
                        f"     Contracts: {file_integrity[filename]['contract_count']}"
                    )
            else:
                logger.error(f"  ❌ {filename}: NOT FOUND")

        all_files_ok = all(
            info["exists"] and info.get("loadable", False)
        ...
        return {
            ...
            "missing_files": [
                f for f, info in file_integrity.items() if not info["exists"]
            ],
        }
    ...
    def run_comprehensive_validation(self) -> Dict[str, any]:
        ...
        try:
            ...
            # Overall assessment
            all_passed = (
                results["file_integrity"]["all_files_ok"]

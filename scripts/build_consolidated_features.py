...
class ConsolidatedFeaturesBuilder:
    ...
    def load_futures_data(self) -> pd.DataFrame:
        ...
        if isinstance(futures_df.index, pd.DatetimeIndex):
            ...
            logger.info(f"Master timeline set: {len(self.master_timeline)} dates from {self.master_timeline.min()} to {self.master_timeline.max()}")
        else:
    ...
    def load_group_features(self) -> pd.DataFrame:
        ...
        group_df["Date"] = pd.to_datetime(group_df["Date"], format="%d/%m/%Y", dayfirst=True)
    ...
    def load_index_features(self) -> pd.DataFrame:
        ...
        index_df = pd.read_csv(
            self.raw_dir / "Raw_65and62_Index.csv",
            encoding="utf-8-sig"
        )
    ...
    def load_weekly_features(self) -> pd.DataFrame:
        ...
        inventory_df = pd.read_csv(
            self.raw_dir / "IOCJ Inventory.csv",
            encoding="utf-8-sig"
        )
        inventory_df["Date"] = pd.to_datetime(inventory_df["Date"], format="%d/%m/%Y", dayfirst=True)
        inventory_df.set_index("Date", inplace=True)
        
        # Load IOCJ Weekly Shipment
        shipment_df = pd.read_csv(
            self.raw_dir / "IOCJ Weekly Shipment.csv",
            encoding="utf-8-sig"
        )
        shipment_df["Date"] = pd.to_datetime(shipment_df["Date"], format="%d/%m/%Y", dayfirst=True, errors='coerce')
        ...
        if shipment_df.index.has_duplicates:
            logger.warning("Found duplicate dates in shipment data, keeping first occurrence")
        ...
        if weekly_df.index.has_duplicates:
            logger.warning("Found duplicate dates in weekly data, keeping first occurrence")
    ...
    def apply_forward_fill(self, weekly_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Forward-fill applied: {original_points} original → {filled_points} daily observations")
    ...
    def align_all_features(
        ...
        consolidated = consolidated.loc[:, ~consolidated.columns.str.contains('^Unnamed')]
        
        # Remove contract month tracking columns (not features for ML)
        feature_columns = [col for col in consolidated.columns if not col.startswith('contract_month')]
        ...
        y_values = (price_65.shift(-1) / price_65).apply(lambda x: np.log(x) * 100 if pd.notna(x) and x > 0 else pd.NA)
        consolidated['Y'] = y_values
        
        logger.info(f"Y values calculated: {y_values.notna().sum()} valid out of {len(y_values)} total")
    ...
    def validate_features(self, df: pd.DataFrame) -> None:
        ...
        if actual_features != expected_features:
            logger.warning(f"Expected {expected_features} columns (12 features + Y), got {actual_features}")
    ...
    def save_results(self, df: pd.DataFrame) -> None:
        ...
        print(f"🎯 Y values: {df['Y'].notna().sum()} valid (last row = NaN as expected)")

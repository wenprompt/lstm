...
def check_ratio_example(contracts, dataset_name):
    ...
    if pd.notna(aug_price) and pd.notna(sep_price):
        ...
        print(
            f"  Difference after adjustment: {abs(aug_price * ratio - sep_price):.8f}"
        )
...
def check_multiple_rollover_examples(contracts, dataset_name):
    ...
    for example in examples[:10]:
        print(
            f"{example['rollover_day'].strftime('%Y-%m-%d')} | "
    ...
    if ratios:
        ...
        print(
            f"  Ratios > 1: {sum(1 for r in ratios if r > 1)} ({100 * sum(1 for r in ratios if r > 1) / len(ratios):.1f}%)"
        )

...
class DataProcessingValidator:
    ...
    def validate_split_ratios(self) -> Dict[str, Any]:
        ...
        if not validation_results["valid_sum"]:
            logger.error(
                f"❌ Split ratios don't sum to 1.0: {total_ratio} (tolerance: {tolerance})"
            )
            raise ValueError(
                f"Invalid split ratios: {train_ratio} + {val_ratio} + {test_ratio} = {total_ratio}"
            )
        ...
        if val_ratio == 0.0:
            ...
            raise ValueError(
                "val_ratio: 0.0 causes empty validation set and MinMaxScaler errors"
            )

        logger.info(
            f"✅ Split ratios valid: {train_ratio:.2f}/{val_ratio:.2f}/{test_ratio:.2f}"
        )
    ...
    def validate_feature_selection(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ Features valid: {len(selected_features)}/{len(available_features)} selected"
        )
    ...
    def validate_data_pipeline(self) -> Dict[str, Any]:
        ...
        pipeline_results = {
            ...
            "y_target_preserved": all(
                "Y" in df.columns for df in [train_df, val_df, test_df]
            ),
            "chronological_order": (
                train_df.index.max() <= val_df.index.min()
                and val_df.index.max() <= test_df.index.min()
            ),
        }
        ...
        logger.info(
            f"   Chronological order: {pipeline_results['chronological_order']}"
        )
    ...
    def validate_scaling_accuracy(self) -> Dict[str, Any]:
        ...
        if self.config["scaling"]["scale_features"]:
            ...
            for dataset_name, ranges in [
                ("train", scaling_results["train_ranges"]),
                ("val", scaling_results["val_ranges"]),
                ("test", scaling_results["test_ranges"]),
            ]:
                if not (0.0 <= ranges["min"] <= ranges["max"] <= 1.0):
                    logger.warning(
                        f"⚠️ {dataset_name} features outside [0,1]: [{ranges['min']:.3f}, {ranges['max']:.3f}]"
                    )

        logger.info("✅ Scaling validation complete")
        logger.info(
            f"   Train range: [{scaling_results['train_ranges']['min']:.3f}, {scaling_results['train_ranges']['max']:.3f}]"
        )
        logger.info(
            f"   Val range: [{scaling_results['val_ranges']['min']:.3f}, {scaling_results['val_ranges']['max']:.3f}]"
        )
    ...
    def validate_sequence_creation(self) -> Dict[str, Any]:
        ...
        val_info = (
            get_data_info(val_loader)
            if len(val_df) > 0
            else {"num_batches": 0, "total_sequences": 0}
        )
        ...
        if actual_shape[1] != expected_seq_len:
            logger.error(
                f"❌ Sequence length mismatch: expected {expected_seq_len}, got {actual_shape[1]}"
            )

        if actual_shape[2] != expected_features:
            logger.error(
                f"❌ Feature count mismatch: expected {expected_features}, got {actual_shape[2]}"
            )
    ...
    def validate_model_compatibility(self) -> Dict[str, Any]:
        ...
        if not compatibility_results["input_size_match"]:
            logger.error(
                f"❌ Input size mismatch: features={expected_input_size}, model={actual_input_size}"
            )
            raise ValueError(
                f"Model input size {actual_input_size} doesn't match {expected_input_size} selected features"
            )
...
def main():
    ...
    print("=" * 50)
    print(
        f"Total samples: {results['pipeline']['actual_splits']['train'] + results['pipeline']['actual_splits']['val'] + results['pipeline']['actual_splits']['test']}"
    )
    ...
    print(
        f"Expected batches - Train: {results['sequences']['train_info']['num_batches']}, Val: {results['sequences']['val_info']['num_batches']}, Test: {results['sequences']['test_info']['num_batches']}"
    )
    print("=" * 50)

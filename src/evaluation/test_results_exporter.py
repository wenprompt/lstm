#!/usr/bin/env python3
"""
Test results exporter for LSTM iron ore price forecasting.

This module exports detailed test results as a DataFrame containing:
...
class TestResultsExporter:
    """Export detailed test results with dates and price information."""
    
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize test results exporter.
    ...
    def load_original_prices(self) -> pd.DataFrame:
        ...
        price_df = pd.DataFrame({
            "raw_price_65_m1": df["raw_price_65_m1"],
            "Y": consolidated_df["Y"]
        })
    ...
    def create_test_results_dataframe(
        ...
        """
        logger.info("Creating comprehensive test results DataFrame...")
        ...
        logger.info("Validating test results alignment...")
        logger.info(f"  Test samples: {len(results_df)}")
        logger.info(f"  Date range: {results_df['date'].min()} to {results_df['date'].max()}")
        logger.info(f"  Raw price range: {results_df['raw_65_m1_price'].min():.2f} to {results_df['raw_65_m1_price'].max():.2f}")
        logger.info(f"  Predicted returns range: {results_df['predicted_ln_returns'].min():.4f} to {results_df['predicted_ln_returns'].max():.4f}")
        logger.info(f"  Actual returns range: {results_df['actual_ln_returns'].min():.4f} to {results_df['actual_ln_returns'].max():.4f}")
    ...
    def export_test_results(
        ...
        """
        Export comprehensive test results to CSV and return metadata.
        ...
        """
        logger.info("Exporting detailed test results...")
        ...
        # Save to CSV
        csv_path = save_dir / "test_results_detailed.csv"
        results_df.to_csv(csv_path, index=False)
        
        # Save to pickle for fast loading
        pkl_path = save_dir / "test_results_detailed.pkl"
        ...
        if isinstance(date_range_info, dict):
            logger.info(f"  Date range: {date_range_info['start']} to {date_range_info['end']}")
        else:
...
def export_test_results(
    ...
    """
    Factory function to export test results.
    ...
    """
    exporter = TestResultsExporter(config)
    return exporter.export_test_results(model, test_loader, device, save_dir)
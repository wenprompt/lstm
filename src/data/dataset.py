...
class LSTMTimeSeriesDataset(Dataset):
    ...
    def __init__(self, dataframe: pd.DataFrame, sequence_length: int):
        ...
        # Handle empty dataframes (e.g., when val_ratio=0)
        if len(dataframe) == 0:
            self.num_samples = 0
        else:
            self.num_samples = max(0, len(dataframe) - sequence_length)
...
def create_dataloaders(
    ...
    logger.info(
        f"Train DataLoader: {len(train_loader)} batches ({len(train_dataset)} sequences)"
    )

    # Handle empty validation set case
    if len(val_dataset) > 0:
        logger.info(
            f"Val DataLoader: {len(val_loader)} batches ({len(val_dataset)} sequences)"
        )
    else:
        logger.info("Val DataLoader: 0 batches (0 sequences, validation disabled)")

    logger.info(
        f"Test DataLoader: {len(test_loader)} batches ({len(test_dataset)} sequences)"
    )
...
def get_data_info(dataloader: DataLoader) -> Dict[str, Any]:
    ...
    """
    # Handle empty dataloader (e.g., when val_ratio=0)
    if len(dataloader.dataset) == 0:  # type: ignore
        return {
            "num_batches": 0,

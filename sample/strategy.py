...
from consts import C_DATE, C_INDEX, C_LONG, C_SHORT, C_M0, C_M1, C_M2, C_M3, C_M4, C_TOTAL
...
def strategy(exit_option: ExitOption, buffer: float, predict_file: str) -> List[SimpleTrade]:
    ...
    df_day = pd.read_excel(EXCEL_FILENAME_PH, sheet_name=0, index_col=0, parse_dates=True, usecols='A', names=[C_DATE])
    df_day = df_day[(df_day.index >= TS_TEST_START) & (df_day.index <= TS_END)].sort_index()
    ...
    for month, df_month in df_ind.groupby(df_ind.index.to_period('M')):
        ...
        for ts, row in df_month.iterrows():
            ...
            if predict_diff > buffer:
                signal_type = C_LONG
                signal_logs = f'{predict_m1:.2f} - {cur_m1} = {predict_diff:.2f} > {buffer}'
            if predict_diff < -buffer:
                signal_type = C_SHORT
                signal_logs = f'{predict_m1:.2f} - {cur_m1} = {predict_diff:.2f} < -{buffer}'
            # exit by back to 0
            for trade in trade_list:
                ...
                if trade.entry_type == C_LONG and predict_diff <= 0:
                    trade.exit_trade(exit_ts=ts, exit_price=exit_price, exit_option=EXIT_REVERSE, exit_signal_logs=f'{predict_m1:.2f} - {cur_m1} = {predict_diff:.2f} <= 0')
                elif trade.entry_type == C_SHORT and predict_diff >= 0:
                    trade.exit_trade(exit_ts=ts, exit_price=exit_price, exit_option=EXIT_REVERSE, exit_signal_logs=f'{predict_m1:.2f} - {cur_m1} = {predict_diff:.2f} >= 0')
            # entry
            if signal_type is None:
                continue
            current_position = sum(trade.entry_size for trade in trade_list if (not trade.is_exit() or trade.exit_ts > ts)) + 1
            if ts in monthly_last:
                ...
                entry_main = f'{m1}'
                entry_price = cur_m1
            trade = SimpleTrade(signal_ts=ts, entry_ts=ts, entry_type=signal_type, entry_size=1, entry_main=entry_main, entry_price=entry_price, entry_signal_logs=signal_logs, current_position=current_position)
        ...
        for trade in trade_list:
            ...
            if exit_option == EXIT_NON_PRICING:
                ...
                trade.exit_trade(exit_ts=ser.index[-1], exit_price=ser.iloc[-1], exit_option=EXIT_EOM, exit_signal_logs=f'EOM on {ser.index[-1].date()}')
                ...
                trade.foo.update({
                    K_EXIT: EXIT_PRICING,
                    K_PRICING_DATE_LIST: ser.index
                })
                trade.exit_trade(exit_ts=ser.index[-1], exit_price=ser.mean().round(ROUND_6), exit_option=EXIT_EOM, exit_signal_logs=f'EOM on {ser.index[-1].date()}')
            else:
...
def cal_mtm(key: str, trade_dto_list: List[SimpleTrade], index_list: List[pd.Timestamp], contract_dict: dict[pd.Period, pd.Series], is_save) -> tuple:
    action_list = []
    trade_list = []
    value_list = []
    for trade in trade_dto_list:
        ...
        # entry
        entry_size = trade.entry_size if trade.entry_type == C_LONG else -trade.entry_size
        entry_size *= C_ENTRY_SIZE_MULTIPLIER_100
        action_list.append({C_DATE: trade.entry_ts, trade.entry_main: entry_size})
        value_list.append({C_DATE: trade.entry_ts, trade.entry_main: entry_size * trade.entry_price})
        # exit
        if trade.is_exit():
            action_list.append({C_DATE: trade.exit_ts, trade.entry_main: -entry_size})
            value_list.append({C_DATE: trade.exit_ts, trade.entry_main: -entry_size * trade.exit_price})
